...
func WithTLS() TunnelOption {
	return newFuncTunnelOption(func(do *TunnelService) {
		...
		tlsConfig, err := xtls.NewClientTLSConfig("", "", "", "")
		if err != nil {
			return
		}
		do.meta["tls"] = true
	...
	return newFuncTunnelOption(func(do *TunnelService) {
		do.afterHooks = append(do.afterHooks, core.AfterHook{
			...
			DialerHook: func(ctx context.Context, c net.Conn, addr string) (context.Context, net.Conn, error) {
				...
				return ctx, cio.WrapConnWithUnderlyingClose(c, wrap), nil
			},
		})
	})
}
	...
	return newFuncTunnelOption(func(do *TunnelService) {
		if do.listener == nil {
			do.afterHooks = append(do.afterHooks, core.AfterHook{
				...
				DialerHook: func(ctx context.Context, c net.Conn, addr string) (context.Context, net.Conn, error) {
					return ctx, cio.WrapConnWithUnderlyingClose(c, wrapper.NewSnappyWrapper(c, c, nil)), nil
				},
			})
			return
		}
...
func WithProxyClient(proxyAddr []string) TunnelOption {
	return newFuncTunnelOption(func(do *TunnelService) {
		do.meta[core.ContextDialerMetaKey] = buildProxyContextDialer(proxyAddr)
	})
}

func buildProxyContextDialer(proxyAddr []string) core.ContextDialer {
	proxies, err := parseProxyURLs(proxyAddr)
	if err != nil {
		return core.NewProxyDialer(func(context.Context, string, string) (net.Conn, error) {
			return nil, err
		})
	}

	proxyDial, err := proxyclient.NewClientChain(proxies)
	if err != nil {
		return core.NewProxyDialer(func(context.Context, string, string) (net.Conn, error) {
			return nil, err
		})
	...
	return core.NewProxyDialer(proxyDial)
}

func parseProxyURLs(proxyAddr []string) ([]*url.URL, error) {
	proxies := make([]*url.URL, 0, len(proxyAddr))
