...
type Stream struct {
	...
	// effectiveMaxWindow is the current window level used by the two-level
	// adaptive flow control. Protected by recvLock.
	effectiveMaxWindow uint32
}

// newStream is used to construct a new stream within
// a given session for an ID
func newStream(session *Session, id uint32, state streamState) *Stream {
	recvWin := initialStreamWindow
	sendWin := initialStreamWindow
	var effMax uint32
	if fc := session.config.FlowControl; fc != nil {
		recvWin = fc.LargeWindow
		sendWin = fc.LargeWindow
		effMax = fc.LargeWindow
	}
	s := &Stream{
		id:                 id,
		session:            session,
...
START:
	if readDeadline := s.readDeadline.Load().(time.Time); !readDeadline.IsZero() && !time.Now().Before(readDeadline) {
		return 0, ErrTimeout
	}
...
func (s *Stream) write(b []byte) (n int, err error) {
	...
	var body []byte
START:
	if writeDeadline := s.writeDeadline.Load().(time.Time); !writeDeadline.IsZero() && !time.Now().Before(writeDeadline) {
		return 0, ErrTimeout
	}
	s.stateLock.Lock()
	switch s.state {
...
WAIT:
	...
	// Flow control auto-reset: if sender is blocked and no WindowUpdate
	// arrives within ConnectionWriteTimeout*WaitN, reset sendWindow.
	var fcTimeout <-chan time.Time
	var fcTimer *time.Timer
	if fc := s.session.config.FlowControl; fc != nil && fc.WaitN > 0 {
		d := s.session.config.ConnectionWriteTimeout * time.Duration(fc.WaitN)
		fcTimer = time.NewTimer(d)
		fcTimeout = fcTimer.C
	}
	...
	case <-timeout:
		...
		return 0, ErrTimeout
	case <-fcTimeout:
		// No WindowUpdate received in time; assume no flow control,
		// reset sendWindow to LargeWindow.
		atomic.StoreUint32(&s.sendWindow, s.session.config.FlowControl.LargeWindow)
	}
	if timer != nil {
		if !timer.Stop() {
			<-timeout
...
// maybeWindowUpdate decides the window level based on buffer pressure
// and sends a window update accordingly. Used when FlowControl is configured.
func (s *Stream) maybeWindowUpdate() error {
	fc := s.session.config.FlowControl
	if fc == nil {
	...
	if bufLen == 0 {
		s.effectiveMaxWindow = fc.LargeWindow
	} else if float64(bufLen)/float64(fc.LargeWindow) >= fc.Threshold {
		s.effectiveMaxWindow = fc.SmallWindow
	}
	s.recvLock.Unlock()

	return s.sendWindowUpdateAdaptive()
}

// sendWindowUpdateAdaptive is like sendWindowUpdate but uses effectiveMaxWindow.
func (s *Stream) sendWindowUpdateAdaptive() error {
	s.controlHdrLock.Lock()
	defer s.controlHdrLock.Unlock()

	s.recvLock.Lock()
	max := s.effectiveMaxWindow
	var bufLen uint32
	if s.recvBuf != nil {
		bufLen = uint32(s.recvBuf.Len())
	}

	// Clamp bufLen so we don't underflow
	if bufLen > max {
		bufLen = max
	}
	available := max - bufLen

	// When downgrading, available may be <= recvWindow; no update needed.
	var delta uint32
	if available > s.recvWindow {
		delta = available - s.recvWindow
	}
	flags := s.sendFlags()
...
func (s *Stream) readData(hdr header, flags uint16, conn io.Reader) error {
	...
	if length > s.recvWindow {
		if fc := s.session.config.FlowControl; fc != nil {
			// Sender auto-reset its window; expand recvWindow to match.
			s.recvWindow = fc.LargeWindow
		} else {
