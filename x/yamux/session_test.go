...
func testFlowControlConf() *Config {
	conf := testConfNoKeepAlive()
	conf.FlowControl = &FlowControlConfig{
		LargeWindow: 256 * 1024,
		SmallWindow: 32 * 1024,
		Threshold:   0.5,
		WaitN:       2,
	}
	conf.MaxStreamWindowSize = 256 * 1024
	return conf
}

// TestFlowControl_BasicTransfer verifies data transfer works with FlowControl enabled.
func TestFlowControl_BasicTransfer(t *testing.T) {
	conf := testFlowControlConf()
	conn1, conn2 := testConnPipe(t)
	client, server := testClientServerConfig(t, conn1, conn2, conf, conf.Clone())
	...
	drainErrorsUntil(t, errCh, 2, 5*time.Second, "basic transfer timeout")
}

// TestFlowControl_WindowDowngrade verifies that under buffer pressure the
// effective window downgrades to SmallWindow.
func TestFlowControl_WindowDowngrade(t *testing.T) {
	conf := testFlowControlConf()
	conn1, conn2 := testConnPipe(t)
	client, server := testClientServerConfig(t, conn1, conn2, conf, conf.Clone())

	// Fill the receiver buffer past the threshold (>50% of LargeWindow).
	flood := int64(conf.FlowControl.LargeWindow)
	errCh := make(chan error, 1)
	...
	go func() {
		...
		// Write enough to fill the window.
	...
	drainErrorsUntil(t, errCh, 1, 5*time.Second, "flood timeout")

	// Partial read: leave >50% in buffer → should downgrade.
	...
	eff := stream.effectiveMaxWindow
	stream.recvLock.Unlock()

	if eff != conf.FlowControl.SmallWindow {
		t.Fatalf("expected SmallWindow %d, got %d", conf.FlowControl.SmallWindow, eff)
	}

	// Drain remaining buffer → should upgrade back.
	_, err = io.Copy(io.Discard, io.LimitReader(stream, flood-1024))
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	...
	if eff != conf.FlowControl.LargeWindow {
		t.Fatalf("expected LargeWindow %d, got %d", conf.FlowControl.LargeWindow, eff)
	}
}

// TestFlowControl_SenderAutoReset verifies that a blocked sender auto-resets
// its sendWindow after the configured timeout.
func TestFlowControl_SenderAutoReset(t *testing.T) {
	conf := testFlowControlConf()
	conf.ConnectionWriteTimeout = 50 * time.Millisecond
	conf.FlowControl.WaitN = 2 // 100ms total
	...
	go func() {
		...
		_, err = stream.Write(make([]byte, conf.FlowControl.LargeWindow))
		if err != nil {
			errCh <- err
			return
		}

		// sendWindow should be ~0 now. The next write should block,
		// then auto-reset after ~100ms and succeed.
		err = stream.SetWriteDeadline(time.Now().Add(2 * time.Second))
		if err != nil {
			errCh <- err
			return
		}
	...
	drainErrorsUntil(t, errCh, 2, 5*time.Second, "sender auto-reset timeout")
}

// TestFlowControl_HighLoadSustained sends a large amount of data while the
// receiver reads slowly, then the sender stops and the receiver drains fast.
// Verifies the full cycle: LargeWindow → SmallWindow → LargeWindow.
func TestFlowControl_HighLoadSustained(t *testing.T) {
	conf := testFlowControlConf()
	conf.ConnectionWriteTimeout = 100 * time.Millisecond
	conf.FlowControl.WaitN = 2

	conn1, conn2 := testConnPipe(t)
	client, server := testClientServerConfig(t, conn1, conn2, conf, conf.Clone())

	totalData := int64(conf.FlowControl.LargeWindow) * 4
	errCh := make(chan error, 2)

	// Track window transitions: 1=saw small, 2=saw large after small.
	var phase int32 // 0→1→2
	...
	go func() {
		...
		for total < totalData {
			...
			if p == 0 && eff == conf.FlowControl.SmallWindow {
				atomic.StoreInt32(&phase, 1)
				t.Logf("[reader] downgrade to SmallWindow at %d/%d bytes", total, totalData)
			} else if p == 1 && eff == conf.FlowControl.LargeWindow {
				atomic.StoreInt32(&phase, 2)
				t.Logf("[reader] upgrade back to LargeWindow at %d/%d bytes", total, totalData)
			}

			if err != nil {
				if err == io.EOF && total == totalData {
					break
				}
				errCh <- fmt.Errorf("read err at %d/%d: %v", total, totalData, err)
				return
			}
			// Slow read during first half, fast drain during second half.
			if total < totalData/2 {
	...
	go func() {
		...
		for written < totalData {
			n, err := stream.Write(chunk)
			written += int64(n)
			if err != nil {
				errCh <- fmt.Errorf("write err at %d/%d: %v", written, totalData, err)
				return
			}
		}
		errCh <- nil
	...
	drainErrorsUntil(t, errCh, 2, 30*time.Second, "high load sustained timeout")

	finalPhase := atomic.LoadInt32(&phase)
	if finalPhase < 1 {
		t.Fatal("flow control never activated (never saw SmallWindow)")
	}
	if finalPhase < 2 {
		t.Fatal("window never recovered to LargeWindow after load dropped")
	}
	t.Logf("[result] full cycle confirmed: LargeWindow → SmallWindow → LargeWindow")
}

// TestFlowControl_LowLoadSustained sends data while the receiver reads
// immediately. Verifies the window stays at LargeWindow (no flow control).
func TestFlowControl_LowLoadSustained(t *testing.T) {
	conf := testFlowControlConf()
	conf.ConnectionWriteTimeout = 100 * time.Millisecond
	conf.FlowControl.WaitN = 2
	...
	go func() {
		...
		for total < totalData {
			...
			stream.recvLock.Unlock()
			if eff == conf.FlowControl.SmallWindow {
				atomic.StoreInt32(&sawSmallWindow, 1)
			}
	...
	// Writer: sends in moderate chunks.
	go func() {
		stream, err := client.OpenStream()
		if err != nil {
			errCh <- err
	...
	drainErrorsUntil(t, errCh, 2, 30*time.Second, "low load sustained timeout")

	if atomic.LoadInt32(&sawSmallWindow) != 0 {
		t.Log("warning: SmallWindow observed under low load (may be transient)")
	}
}

// TestFlowControl_BidirectionalDeadlock verifies that when both sides write
// simultaneously without reading, the fcTimeout auto-reset prevents deadlock.
func TestFlowControl_BidirectionalDeadlock(t *testing.T) {
	conf := testFlowControlConf()
	conf.ConnectionWriteTimeout = 50 * time.Millisecond
	conf.FlowControl.WaitN = 2 // 100ms auto-reset

	conn1, conn2 := testConnPipe(t)
	client, server := testClientServerConfig(t, conn1, conn2, conf, conf.Clone())

	payload := make([]byte, conf.FlowControl.LargeWindow)
	errCh := make(chan error, 2)

	// Both sides: write to exhaust window, then read to drain.
	doWork := func(local *Session, remote *Session, open bool) {
		var stream *Stream
		var err error
		if open {
			stream, err = local.OpenStream()
		} else {
			stream, err = local.AcceptStream()
		}
		if err != nil {
			errCh <- err
			return
		...
		// Write in background — will block when window exhausts.
		...
		// Read everything the peer sends.
	...
	go doWork(client, server, true)
	go doWork(server, client, false)

	drainErrorsUntil(t, errCh, 2, 10*time.Second, "bidirectional deadlock timeout")
}

// TestFlowControl_WindowExhaustedWriteBlocked is the worst-case scenario:
// both sides exhaust their sendWindow, AND the underlying write path is
// temporarily blocked so WindowUpdate cannot be delivered. Verifies that
// after the write path unblocks, the fcTimeout auto-reset recovers the
// stream without deadlock.
func TestFlowControl_WindowExhaustedWriteBlocked(t *testing.T) {
	conf := testFlowControlConf()
	conf.ConnectionWriteTimeout = 50 * time.Millisecond
	conf.FlowControl.WaitN = 2 // 100ms auto-reset
	...
	// Server: accept, write to fill window, then read.
	go func() {
		stream, err := server.AcceptStream()
		if err != nil {
			errCh <- err
		...
		// Write to exhaust server→client sendWindow.
		_ = stream.SetWriteDeadline(time.Now().Add(5 * time.Second))
		go func() { stream.Write(payload) }()

		// Read to drain client→server data.
		_ = stream.SetReadDeadline(time.Now().Add(5 * time.Second))
	...
	go func() {
		...
		// Write to exhaust client→server sendWindow.
		_ = stream.SetWriteDeadline(time.Now().Add(5 * time.Second))
		go func() { stream.Write(payload) }()

		// Wait for both sides to be blocked.
		time.Sleep(50 * time.Millisecond)

		// Block BOTH write paths — no WindowUpdate can be sent.
		clientPipe := conn1.(*pipeConn)
		serverPipe := conn2.(*pipeConn)
		clientPipe.writeBlocker.Lock()
		serverPipe.writeBlocker.Lock()

		// Hold the block for longer than fcTimeout (100ms).
		time.Sleep(200 * time.Millisecond)

		// Unblock — fcTimeout should fire and recover.
		serverPipe.writeBlocker.Unlock()
	...
	drainErrorsUntil(t, errCh, 2, 10*time.Second,
		"window exhausted + write blocked: deadlock detected")
}
